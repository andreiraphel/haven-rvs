...
def predict(req: PredictRequest):
    try:
        ...
        c1_s = [CODE_MAP.get(v.get("building_code"), 2), get_max(v.get("plan_irregularity"), PLAN_MAP), VERT_MAP.get(v.get("vertical_irregularity"), 1), PROX_MAP.get(v.get("building_proximity"), 1), 3 if stories >= 3 else stories, get_max(v.get("structural_material"), MAT_MAP), 1 if bays >= 5 else 2 if bays >= 3 else 3, 1 if spacing < 3 else 2 if spacing <= 5 else 3, get_max(v.get("building_enclosure"), ENCL_MAP), get_max(v.get("wall_material"), WALL_MAP), get_max(v.get("structural_framing_type"), FRAME_MAP), get_max(v.get("flooring_material"), FLOOR_MAP)]
